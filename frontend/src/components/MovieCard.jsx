...
// src/components/MovieCard.jsx
// ─────────────────────────────────────────────
// Netflix-style movie card with real TMDB poster
// - Shows real poster image if available
// - Falls back to colored placeholder if no poster
// - Hover: scales up, shows rating + genre overlay
// ─────────────────────────────────────────────
...
export default function MovieCard({ movie, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* ── Gradient overlay — always on top of poster ── */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/10 to-transparent
                      group-hover:via-black/30 transition-all duration-300" />

      {/* ── Movie title + hover info ── */}
      <div className="absolute bottom-0 left-0 right-0 p-2">
      ...
      {/* ── Favourite button on hover ── */}
      <button
        onClick={(e) => e.stopPropagation()}
        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100
                   transition-opacity bg-black/60 hover:bg-netflix-red
