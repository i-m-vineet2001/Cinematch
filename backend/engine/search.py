...
import math
import pandas as pd
from engine.tmdb import get_poster_url, get_poster_urls_batch
...
def get_movies_by_genre(genre: str, limit: int = 25) -> list:
    """
    Filter movies by genre sorted by popularity.
    Default limit is 25 — enough to fill the page without long wait.
    """
    results = df[df["genres"].str.lower().str.contains(genre.lower(), na=False)]
    sorted_results = results.sort_values("popularity", ascending=False)
    if limit is not None:
...
def _format_movies(df_slice) -> list:
    ...
    Fetches ALL poster URLs in parallel first — much faster than one by one.
    """
    rows = list(df_slice.iterrows())
    if not rows:
        return []

    # Collect all titles and fetch posters in one parallel batch
    titles = [safe_str(row.get("title"), "Unknown") for _, row in rows]
    posters = get_poster_urls_batch(titles)  # ← parallel fetch

    return [
        _format_movie(row, poster_url=posters.get(safe_str(row.get("title"), ""), ""))
        for _, row in rows
    ]


def _format_movie(row, poster_url: str = None) -> dict:
    """
    Convert a single DataFrame row into a JSON-safe dict.
    poster_url can be passed in (from batch) or fetched individually.
    """
    title = safe_str(row.get("title"), "Unknown")
    return {
        "title": title,
        ...
        "cast": safe_str(row.get("cast")),
        "keywords": safe_str(row.get("keywords")),
        # Use passed poster_url (from batch) or fetch individually as fallback
        "poster_url": poster_url if poster_url is not None else get_poster_url(title),
    }